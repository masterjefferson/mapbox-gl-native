...
std::shared_ptr<const mbgl::SpriteImage> toSpriteImage(const QImage &sprite) {
    ...
    return std::make_shared<mbgl::SpriteImage>(mbgl::PremultipliedImage
...
void QMapboxGL::addIcon(const QString &name, const QImage &sprite)
{
    if (sprite.isNull()) return;

    d_ptr->mapObj->addIcon(name.toStdString(), toSpriteImage(sprite));
}
